...
namespace lua {
  ...
  struct state : state_base {
    ...
    inline push(const std::tuple<Tp...>& t) {
    }
    ...
    inline void push(const tuple_t& t) {
      push<0, tuple_t>(t);
    }
    ...
    template <typename tuple_t>
    tuple_t inline get_values(int idx = -1) {
      return get_values_<0, tuple_t>(idx);
    }

    template <typename T>
    T get_value(const int idx = -1) {
      return stack_reader<T>::read(*this, idx);
    }
    
    template <typename T>
    void get_value_into(T& v, const int idx = -1) {
      v = get_value<T>(idx);
    }
    ...
    template <std::size_t I = 0, typename tuple_t>
    typename std::enable_if<0 == std::tuple_size<tuple_t>::value, tuple_t>::type
    inline get_fields_(const std::vector<std::string>& keys, int idx = -1) {
      return std::tuple<>();
    }
    ...
    template <typename tuple_t>
    tuple_t inline get_fields(const std::vector<std::string>& keys, int idx = -1) {
    ...
    template <typename T>
    T get_field(const char* key, const int idx = -1) {
      getfield(idx, key);
    ...
    template <typename T>
    void get_field_into(const char* key, T& v, const int idx = -1) {
      v = get_field<T>(key, idx);
    }

    // Read array-like tables
    template <typename row_t>
    std::vector<row_t> get_rows(const int idx = -1, const size_t size_hint = 0) {
    ...
    template <typename row_t>
    void get_rows_into(std::vector<row_t>& table, const int idx = -1, const size_t size_hint = 0) {
